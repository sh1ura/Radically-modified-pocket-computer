...
* ショットキーバリアダイオード
  ...
  - ちょっと大きめですが、以前購入したストック（[これ](https://www.amazon.co.jp/dp/B07ZQM1Q4N)）を使用しました。
  - Raspberry Pi Zero を搭載しない場合は不要です
* L字ピンヘッダ、ピンソケットケーブル

### プロジェクト全体に必要な部品

ESP32-S3 だけを用いた端末に仕上げることも出来ます。その場合は Raspberry Pi Zero は不要です。
...
* Raspberry Pi Zero
  - このプロジェクトでは Raspberry Pi Zero 2 W を搭載していますが、初代の Zero 等でも問題ないはずです。 
  - 強力両面テープで貼り付けてあるだけです。
