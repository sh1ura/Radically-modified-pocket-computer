...
boolean bleInit = false; // BLE is ready
...
void loopRpi(struct KeyPos key) {
  char c;
  char c2[2] = " ";
...
void loopESP32(struct KeyPos keyRaw) {
  double x, y;
  char key;
  boolean dispFlag = false;
  boolean shift = false;
...
void bleLoop(struct KeyPos keyRaw) {
  char key;
  char str[2] = " ";
  static int ctrlStatus = false;

  if (!bleKeyboard.isConnected()) {
    return;
  }

  if(keyRaw.ctrl != ctrlStatus) {
    keyRaw.ctrl ? bleKeyboard.press(KEY_LEFT_CTRL) : bleKeyboard.release(KEY_LEFT_CTRL);
    ctrlStatus = keyRaw.ctrl;
  }
  keyRaw.ctrl = false; // suppress control code generation
  key = keyChar(keyRaw);
  if (key) {
    switch (key) {
      case 0x1b:  // esc
...
boolean checkBleMode(struct KeyPos keyRaw) {
  static boolean bleKbdMode = false;

  if (keyRaw.row == 9 && keyRaw.clm == 1) {  // 2ndf key
    bleKbdMode = !bleKbdMode;
    turnOffDisplay();
    messageBox(bleKbdMode ? "BLE keyboard mode" : "Resume");
    if (!bleInit) {
      bleKeyboard.begin();
      bleInit = true;
    }
