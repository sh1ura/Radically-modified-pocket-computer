...
![IMG_20251129_002059279](https://github.com/user-attachments/assets/9773236f-2edd-45e8-acf0-9e249d49d508)

* UART で Raspberry Pi と通信
* 全キーのスキャン（押下状態を取得）
  - 一部については文字コードを生成
  - shift (主に記号入力), ctrl (BASICキー), caps (大文字入力), ESC (カナキー)
* VT100サブセットの ANSI エスケープシーケンス処理
  - 必要最低限のコードのみ処理しています
    - emacs 等では、非対応のコードのため、時々表示がずれることがあります
  - カラー・反転等には対応していません
  - 42列7行表示
    - ログイン後、`stty rows 7 cols 41` のように画面サイズをセットして使用してください
  - 履歴表示（上へのスクロール）
    - CONSTキー：上へスクロール　ANSキー : 下へスクロール
    - なにかのキーを押すと最下行へ復帰

### RPN 関数電卓モード（ESP32 単体動作時）
![IMG_20251129_002216310](https://github.com/user-attachments/assets/ae766770-252a-4725-836f-81186ca1cf08)
