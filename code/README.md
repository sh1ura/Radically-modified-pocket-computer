...
* 計算結果の bluetooth 転送機能（キーボードデバイスとして動作）

### 共通

* **Bluetooth キーボード機能**
  - 2nd Fキーの押下で、いつでも、通常の動作状態と Bluetooth キーボード状態との間を行き来できます
  - ターミナルモード（Raspberry Pi 起動時）では、GUI画面を表示しているときのキー入力に利用できます（VNC利用時、HDMIディスプレイ接続時等）
  - RPN 関数電卓モード（ESP32 単体動作時）では、一般の bluetooth キーボードとしての動作に加え、電卓表示中も TAB キーを押すことで計算結果（スタックトップ）の数値をPCへ入力できます
* 10秒間操作しないと、ESP32 が間欠動作になります
  - ディスプレイが点いたままなので、表示文字数に応じて電力消費があります
* 30 秒間操作しないと、さらに画面が消灯します
  - RPN 関数電卓モードの場合、消費電力は 10mA 以下になります。
  - RPi モードのときは、待機中の消費電力は約 140mA になります。
* キーが押されると復帰します
